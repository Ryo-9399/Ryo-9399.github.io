...
v1.00.0
	**第3版マップデータ策定。**
	マップデータの使用方法については
	https://github.com/Ryo-9399/mc_canvas/wiki/%E7%AC%AC3%E7%89%88%E3%83%9E%E3%83%83%E3%83%97%E3%83%87%E3%83%BC%E3%82%BF%E4%BD%BF%E7%94%A8%E6%96%B9%E6%B3%95
	をご覧ください。
	無限に敵を配置できるように改造。
	無限に仕掛けパーツを配置できるように改造。
	マップサイズ自由化。
	同時に配置できる敵・仕掛けパーツの種類数制限を解放。
...
!ReadMe.txt
	本ファイルです。
MasaoJSS使用法まとめ.html
	MasaoJSSの使用法をまとめた簡単なHTMLドキュメントです。
ソースコンバータ.html
...
・advance-map
	第3版マップデータオブジェクト、またはJSONファイルのURL
	使用法は
	https://github.com/Ryo-9399/mc_canvas/wiki/%E7%AC%AC3%E7%89%88%E3%83%9E%E3%83%83%E3%83%97%E3%83%87%E3%83%BC%E3%82%BF%E4%BD%BF%E7%94%A8%E6%96%B9%E6%B3%95
	をご覧ください。
...
●終了方法
new CanvasMasao.Game(...)での戻り値（オブジェクト）のメソッドkill()を
呼び出すと、まさおコンストラクションを終了することができます。

使用例：
	var hoge = new CanvasMasao.Game( … );
	…
	hoge.kill();

この機能は、置換型構築方法（replace）では使用できません。
...
備考：
	...
	追加するならば、このようにスクリプトへ記述します。
		CanvasMasao.Game.pad.coords.push([320, 5, 490, 5, 490, 25, 320, 25]);
		CanvasMasao.Game.pad.chars.push("C");
		CanvasMasao.Game.pad.codes.push(67);
	・styleで指定できる色は、CSSで指定できる色と同じです。
	・CanvasMasao.Game.padへ直接オブジェクトを代入して
...
●更新履歴
v0.05.7
	一言メッセージ等のセリフのフォントをv0.05.5以前のものに戻した
	【JSS】setMyDirectionメソッドを追加
	【JSS】ポーズ操作のメソッド（pause(f), isPaused()）を追加
v0.05.6
	Chrome47（開発段階）で、replaceメソッドが動作しないことを回避
	フォントをJava版まさおに合わせて変更
	CanvasMasao.Game#killメソッドの追加
	乱数生成器を、シードを設定できるように変更
	（将来、ゲームのリプレイの保存に役立つ？）
v0.05.5
...
●連絡先・注意事項
　バグ報告やご意見、ご要望はTwitterまで連絡をお願いします。
Twitterアカウント: @mc_canvas_svc（サポート用アカウント）
